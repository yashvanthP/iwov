...
public class XLSXToDCRs {
	...
	public static void main(String[] args) {
		...
		if (args.length < 5) {
			...
			String username = args[0]; String xslx = args[1]; String vpathQA = args[2]; String vpathTopic = args[3]; String cssdk = args[4]; 
			String password = "";
			if (!vpathQA.endsWith("/")) {
				vpathQA = vpathQA + "/";
			}
			...
			if (null != client) {
				// topicContents has modul as key, then HashMap <String, Integer> with sub-modul as key and num of QA DCRs as value
				...
				// localisedModules has modul/sub-modul as key, then HashMap <String, String> with short lang as key and localised modul as value
				...
				if (null != rootDirQA) {
					// This String array contains the headings extracted from the Excel sheet to use as the element names
					// Each String is converted to lower case and spaces are replaced with underscore characters
		        ...
		        if ((null != topicContents) && (null != rootDirTopic)) {
			        ...
			        if ((null != topicGenericContents) && !topicGenericContents.isEmpty()) {
		        		Document doc = generateTopicDocument(vpathTopic, "Generic", 0, "generic", null);
	...
	public static String convertRTSCellToXML(XSSFRichTextString rtsCellValue) {
		...
		for (int i = 0; i < rtsCellValue.length(); i++) {
			...
			if (!inBold) {
				if (((null != currentFont) && currentFont.getBold()) && ((null == previousFont) || !previousFont.getBold())) {
					if (!lookAheadForBoldChange(rtsCellValue, i + 1, 3) && !inInternalLink && !inExternalLink && !inAnchorLink) {
						sb.append("<Bold style=\"highlight_text\">"); inBold = true;
					}
				}
			}
			...
			if (!currentARGBHexColour.equalsIgnoreCase(previousARGBHexColour)) {
				...
				if (!lookAheadForColourChange(rtsCellValue, currentARGBHexColour, i + 1, 3)) {
					if (currentARGBHexColour.equalsIgnoreCase(LINK_ARGB_INTERNAL)) {
						...
						sb.append("<Span xml:id=\"FORMIDTODO\"><AnchorLink reference=\"javascript:void(0)\">"); inAnchorLink = true;
					}
				}
			}
			...
			if (restOfStr.startsWith("\n")) {
				...
				if (inParagraph) {
					sb.append("</Paragraph>"); inParagraph = false;
				} else if (inList) {
					if ((restOfStrCleaned.length() > 2) && restOfStrCleaned.substring(0, 2).matches("[2-9]\\.")) {
						sb.append("</Item>");
					} else {
						sb.append("</Item></OrderedList>"); inList = false;
					}
				}
			}
	...
	private static boolean writeXML(String path, Document doc, CSClient client, String lang, String type) {
		try {
			...
			if (null == theFile) {
				// File doesn't yet exist, let's create it and set the TeamSite Extended Attributes
				...
				CSExtendedAttribute[] eas = new CSExtendedAttribute[] {
					new CSExtendedAttribute("G11N/Locale", lang),
					new CSExtendedAttribute("G11N/Localizable", "true"),
					new CSExtendedAttribute("TeamSite/Assocation/Version", "1"),
					new CSExtendedAttribute("TeamSite/Templating/DCR/Type", type),
					new CSExtendedAttribute("iw_form_valid", "true")};
